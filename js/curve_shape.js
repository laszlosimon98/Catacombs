class CurveShape {
    arr = ["UP-RIGHT", "DOWN-RIGHT", "DOWN-LEFT", "UP-LEFT"];
    shape;
    ...
    updatePos(i, j) {
        this.i = i;
        this.j = j;
        this.x = i * this.len * this.size;
        this.y = j * this.len * this.size;
    }
    ...
    init() {
        ...
        for (let i = 0; i < this.len; i++) {
            ...
            for (let j = 0; j < this.len; j++) {
                switch(this.facing) {
                    case "UP-RIGHT":
                        if ((i <= 1 && j >= this.len - 2) || j <= 1 || i >= this.len - 2) {
                            arr[i].push(0);
                        } else {
                            arr[i].push(1);
                        }
                        break;
                    case "UP-LEFT":
                        if (j >= this.len - 2 || i >= this.len - 2 || (i <= 1 && j <= 1)) {
                            arr[i].push(0);
                        } else {
                            arr[i].push(1);
                        }
                        break;
                    case "DOWN-RIGHT":
                        if (i <= 1 || j <= 1 || (j >= this.len - 2 && i >= this.len - 2)) {
                            arr[i].push(0);
                        } else {
                            arr[i].push(1);
                        }
                        break;
                    case "DOWN-LEFT":
                        if (i <= 1 || j >= this.len - 2 || (i >= this.len - 2 && j <= 1)) {
                            arr[i].push(0);
                        } else {
                            arr[i].push(1);
                        }
